...
def main():
    ...
    if st.button("Ask") and q:
        ...
        for i, (m, score) in enumerate(hits, 1):
            with st.expander(f"Source {i} — Page {m.page + 1} (Relevance: {score:.2f})"):
                full_text = load_doc_text(m.doc_id)
                snippet = full_text[m.start:m.end] if full_text else m.preview
